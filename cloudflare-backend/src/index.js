...
async function handleProvider(request, env) {
  ...
  if (url.pathname === '/api/services/provider/my-services' && request.method === 'GET') {
    const services = await env.DB.prepare('SELECT * FROM services WHERE provider_id = ? ORDER BY created_at DESC').bind(user.id).all();
  ...
  if (url.pathname === '/api/services/provider/bookings' && request.method === 'GET') {
    const bookings = await env.DB.prepare(`
      SELECT b.*, s.name as service_name, u.first_name as customer_first, u.last_name as customer_last, u.phone
      FROM bookings b
...
async function handleReviews(request, env) {
  ...
  if (url.pathname === '/api/reviews/my-reviews' && request.method === 'GET') {
    ...
    const avg = await env.DB.prepare('SELECT AVG(provider_rating) as avgRating, COUNT(*) as total FROM reviews WHERE provider_id = ?').bind(user.id).first();
