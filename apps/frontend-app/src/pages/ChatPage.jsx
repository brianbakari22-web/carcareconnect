...
function ChatPage({ user, onClose }) {
  ...
  const fetchInbox = async () => {
    try {
      const res = await fetch('https://carcare-api.brianbakari22.workers.dev/api/chat/inbox', {
  ...
  const fetchSentMessages = async () => {
    try {
      const res = await fetch('https://carcare-api.brianbakari22.workers.dev/api/chat/sent', {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      const data = await res.json();
      if (data.success) {
  ...
  const fetchConversation = async (userId, userName, userRole, bookingId) => {
    ...
    try {
      const url = `https://carcare-api.brianbakari22.workers.dev/api/chat/conversation/${userId}`;
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const res = await fetch('https://carcare-api.brianbakari22.workers.dev/api/chat/send', {
        method: 'POST',
        headers: {
